...
int segment_tree_max_lambda(const int &a, const int &b) {
	return max(a, b);
}
...
int main() {
	...
	SegmentTree<int> tree(arr, segment_tree_max_lambda, 0);
	
	int requests;
	cin >> requests;

	vector< pair<int, int> > ans(requests);
	for (int i = 0; i < requests; ++i) {
		int a, b;
		cin >> a >> b;

		int maximum = tree.get_segment(--a, --b);
		cout << maximum << ' ';
