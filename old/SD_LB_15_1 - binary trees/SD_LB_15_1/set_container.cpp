...
template <class S>
set_container<S>::set_container(int(*setCompare)(S a, S b))
{
	root = nullptr;
	sizeOfSet = 0;
	compare = setCompare;
...
template<class S>
int set_container<S>::defaultCompare(S a, S b)
{
	return a - b;
}
...
void set_container<S>::privateInsert(S data, Node<S>*& currentRoot)
{
	...
	if (compare(data, currentRoot->data) > 0) {
...
template <class S>
bool set_container<S>::erase(S elem, int (*setCompare)(S a, S b)) {
	unsigned oldSize = sizeOfSet;
	
	if (setCompare != NULL) {
		int(*oldCompare) (S a, S b) = compare;
		compare = setCompare;
...
template <class S>
bool set_container<S>::erase(S elem, unsigned quantity, int (*setCompare)(S a, S b)) {
	unsigned oldSize = sizeOfSet;
...
set_container<S>::Node<S>*& set_container<S>::findNode(S data, Node<S>*& currentRoot) {
	...
	int compared = compare(data, currentRoot->data);
