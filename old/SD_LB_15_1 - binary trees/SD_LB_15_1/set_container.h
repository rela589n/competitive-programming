...
public:
	...
	set_container(int(*setCompare)(S a, S b) = defaultCompare);
	~set_container();
	...
	bool erase(S, int(*setCompare)(S a, S b) = NULL);
	bool erase(S, unsigned, int(*setCompare)(S a, S b) = NULL);
	bool eraseAll(S);
	void clear();
...
private:
	...
	int(*compare)(S a, S b);
	static int defaultCompare(S a, S b);
